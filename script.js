...
const ADMIN_PASSWORD = 'lakshayjaat57p';
const ADMIN_MIN_PASSWORD_LENGTH = 8;
const ADMIN_MAX_ATTEMPTS = 5;
const ADMIN_LOCKOUT_MS = 120000;
const ADMIN_SESSION_DURATION_MS = 60 * 60 * 1000;
const ADMIN_LOCKED_UNTIL_KEY = 'lakzyrox-admin-locked-until';
const ADMIN_ATTEMPTS_KEY = 'lakzyrox-admin-attempts';
...
const readAdminAttemptState = () => {
  const attempts = Number(sessionStorage.getItem(ADMIN_ATTEMPTS_KEY) || '0');
  const lockedUntil = Number(sessionStorage.getItem(ADMIN_LOCKED_UNTIL_KEY) || '0');
  return { attempts, lockedUntil };
};

const updateAdminAttemptState = (attempts, lockedUntil) => {
  sessionStorage.setItem(ADMIN_ATTEMPTS_KEY, String(attempts));
  sessionStorage.setItem(ADMIN_LOCKED_UNTIL_KEY, String(lockedUntil));
};
...
const unlockAdmin = () => {
  ...
  const now = Date.now();
  const { attempts, lockedUntil } = readAdminAttemptState();

  if (lockedUntil > now) {
    const remainingSeconds = Math.ceil((lockedUntil - now) / 1000);
    setAdminError(`Admin locked for ${remainingSeconds}s.`);
    return;
  }

  if (lockedUntil > 0 && lockedUntil <= now) {
    updateAdminAttemptState(0, 0);
  }

  if (enteredPassword.length < ADMIN_MIN_PASSWORD_LENGTH) {
    setAdminError('Password must be at least 8 characters.');
    return;
  }

  if (enteredPassword === ADMIN_PASSWORD) {
    sessionStorage.setItem('lakzyrox-admin', 'true');
    sessionStorage.setItem('lakzyrox-admin-expires-at', String(Date.now() + ADMIN_SESSION_DURATION_MS));
    sessionStorage.setItem(ADMIN_ATTEMPTS_KEY, '0');
    sessionStorage.setItem(ADMIN_LOCKED_UNTIL_KEY, '0');
    setAdminAccess(true);
    return;
  }
  ...
  if (nextAttempts >= ADMIN_MAX_ATTEMPTS) {
    const blockedUntil = Date.now() + ADMIN_LOCKOUT_MS;
    updateAdminAttemptState(ADMIN_MAX_ATTEMPTS, blockedUntil);
    setAdminError('Too many attempts. Admin locked for 2 minutes.');
    if (adminPasswordInput) {
      adminPasswordInput.value = '';
      adminPasswordInput.disabled = true;
      unlockAdminButton.disabled = true;
    }
    return;
  }

  updateAdminAttemptState(nextAttempts, 0);
  if (adminGateError) {
    adminGateError.hidden = false;
    adminGateError.textContent = `Incorrect password. ${ADMIN_MAX_ATTEMPTS - nextAttempts} attempts left.`;
  }
...
const renderAdminSection = (sectionName) => {
  ...
  list.innerHTML = '';
  if (!items.length) {
    list.innerHTML = '<p style="color:var(--muted);">No items yet.</p>';
    return;
  }
  items.forEach((item) => {
    ...
    row.innerHTML = `
      <div>
        ...
        <p style="margin:0.25rem 0 0; color:var(--muted);">${item.content || item.description || item.url || item.link || ''}</p>
      </div>
      <div style="display:flex; gap:0.45rem; flex-wrap:wrap;">
        <button class="pill-btn" type="button" data-admin-edit="${sectionName}" data-id="${item.id}">Edit</button>
        <button class="pill-btn" type="button" data-admin-private="${sectionName}" data-id="${item.id}">${item.isPublic === false ? 'Public' : 'Private'}</button>
        <button class="pill-btn" type="button" data-admin-delete="${sectionName}" data-id="${item.id}">Delete</button>
      </div>
    `;
  ...
  list.querySelectorAll('[data-admin-edit]').forEach((button) => {
    button.addEventListener('click', () => {
      const selectedItem = items.find((item) => item.id === button.getAttribute('data-id'));
      adminSelection[sectionName] = selectedItem.id;
      populateAdminForm(sectionName, selectedItem);
    });
  });
  list.querySelectorAll('[data-admin-private]').forEach((button) => {
    button.addEventListener('click', () => {
      const selectedItem = items.find((item) => item.id === button.getAttribute('data-id'));
      const updatedItems = items.map((entry) => entry.id === selectedItem.id ? { ...entry, isPublic: entry.isPublic === false } : entry);
      const contentCopy = getStoredContent();
      setSectionItems(sectionName, contentCopy, updatedItems);
      saveContent(contentCopy);
      renderContent();
  ...
  list.querySelectorAll('[data-admin-delete]').forEach((button) => {
    button.addEventListener('click', () => {
      const selectedItem = items.find((item) => item.id === button.getAttribute('data-id'));
      const updatedItems = items.filter((entry) => entry.id !== selectedItem.id);
      const contentCopy = getStoredContent();
      setSectionItems(sectionName, contentCopy, updatedItems);
      saveContent(contentCopy);
      renderContent();
      renderAdminSection(sectionName);
      if (adminSelection[sectionName] === selectedItem.id) delete adminSelection[sectionName];
    });
  });
};

const saveAdminSection = (sectionName) => {
  const config = adminSections[sectionName];
  if (!config) return;
  const content = getStoredContent();
  const items = getSectionItems(sectionName, content);
  const selectedId = adminSelection[sectionName];
  const existing = items.find((item) => item.id === selectedId) || null;
  ...
  config.fields.forEach((field) => {
    ...
    if (field.key === 'platforms') {
      nextItem.platforms = rawValue ? rawValue.split(',').map((entry) => entry.trim()).filter(Boolean) : ['Instagram'];
    } else {
      nextItem[field.key] = rawValue;
    }
  });
  if (!existing) {
    nextItem.id = createId();
    items.push(nextItem);
  } else {
    const index = items.findIndex((item) => item.id === selectedId);
    items[index] = nextItem;
  }
  setSectionItems(sectionName, content, items);
  saveContent(content);
  renderContent();
  renderAdminSection(sectionName);
  adminSelection[sectionName] = nextItem.id;
  populateAdminForm(sectionName, nextItem);
};
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const adminExpiresAt = Number(sessionStorage.getItem('lakzyrox-admin-expires-at') || '0');
  const isAdminUnlocked = sessionStorage.getItem('lakzyrox-admin') === 'true' && Date.now() < adminExpiresAt;

  if (!isAdminUnlocked && adminExpiresAt && Date.now() >= adminExpiresAt) {
    sessionStorage.setItem('lakzyrox-admin', 'false');
    sessionStorage.removeItem('lakzyrox-admin-expires-at');
  }

  setAdminAccess(isAdminUnlocked);
